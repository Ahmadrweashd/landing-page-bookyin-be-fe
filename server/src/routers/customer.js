...
router.get("/services", getCustomersServices);
...
router.post("/", isAuthenticated, upload.fields([{ name: "profileURL", maxCount: 1 }, { name: "backgroundImageURL", maxCount: 1 },]), parseServicesToArray, createCustomerValidation, validate, createCustomer);
router.post("/convert-request/:id", isAuthenticated, findRequest, convertRequestToCustomer);

router.put("/:id", isAuthenticated, upload.fields([{ name: "profileURL", maxCount: 1 }, { name: "backgroundImageURL", maxCount: 1 },]), parseServicesToArray, updateCustomerValidation, validate, findCustomer, updateCustomer);
